...
def get_attention_by_layer(csv_path, language, source, fields):
    ...
    转成 DataFrame，并且只保留你关心的字段（不做归一化）。
...
def plot_language_averaged_with_individuals(
    csv_path,
    languages,
    source="total",
    ...
    """
    1. 画每种语言的曲线（细 + 透明）作为背景：
       - 颜色根据 field 区分
    2. 计算每层在所有语言上的平均 attention（只对指定 fields），
       再画两条粗线：avg-problem、avg-translation。

    - x 轴: layer（1 = 最浅层）
    """
    os.makedirs(output_dir, exist_ok=True)

    per_lang_matrices = []  # 每个语言的 (num_layers, num_fields) 矩阵
    num_layers_ref = None
    field_order = None
    ...
    for lang in languages:
        ...
        per_lang_matrices.append(attn_df.to_numpy())
    ...
    per_lang_matrices = np.stack(per_lang_matrices, axis=0)  # (num_lang, num_layers, num_fields)
    avg_over_langs = per_lang_matrices.mean(axis=0)          # (num_layers, num_fields)
    ...
    plt.title(
        f"Language-wise curves (faint) + cross-language average (bold)\n"
    ...
    save_name = f"avg_over_langs_with_individuals_{source}_{'_'.join(fields)}_{model_name}.png"
...
if __name__ == "__main__":
    ...
    plot_language_averaged_with_individuals(
        csv_path=csv_path,
        languages=languages,
        source="total",
        fields=("problem", "translation"),
