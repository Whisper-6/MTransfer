...
def parse_args():
    ...
    args.output_dir = os.path.join("output", args.model, "QxAen-QenAen", mask_set)
    ...
    def run_generate(data, max_new_tokens, batch_size, data_unfinished):
        ...
        for start in range(0, len(data), batch_size):
            ...
            for i, ex in enumerate(batch):
                ...
                t_start += left_pad
                t_end += left_pad
                spans.append((left_pad, t_start, t_end))
            ...
            for i, ex in enumerate(batch):
                left_pad = spans[i][0]
                prompt_ids = padded_input_ids[i, left_pad:prompt_len]
                ...
                prompt = tokenizer.decode(prompt_ids, skip_special_tokens=False).strip()
...
def build_token_spans(ex, tokenizer):
    ...
    t_char_start, t_char_end = find_span(TRANSLATION_PROMPT, "\n\n")
