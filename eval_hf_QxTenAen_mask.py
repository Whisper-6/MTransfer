...
parser.add_argument("--mask-layers", type=int, default=0, help="前多少层的翻译内容需要遮蔽")
...
output_path = os.path.expanduser(os.path.join(args.out_dir, args.model, "QxTenAen", f"L{args.mask_layers}"))
...
def generate_batch(model, tokenizer, batch, device, temperature=0.3, do_sample=False, mask_layers=0):
    results = []
    ...
    # 获取模型的隐藏层维度
    hidden_size = model.config.hidden_size  # 这里直接从 config 中获取 hidden_size

    # 如果需要mask layers，修改 attention_mask
    if mask_layers > 0:
        # 遮蔽前 mask_layers 层
        for layer_idx in range(mask_layers):
            attention_mask[:, :, layer_idx] = -float('inf')

    outputs = model.generate(**inputs, generation_config=gen_config, attention_mask=attention_mask)
...
# ----------------------
def run_on_gpu(gpu_id, problems, batch_size, temperature, do_sample, mask_layers, return_list):
    ...
    for batch in tqdm(dataloader, desc=f"GPU-{gpu_id}", position=gpu_id):
        batch_results = generate_batch(model, tokenizer, batch, device, temperature, do_sample, mask_layers)
