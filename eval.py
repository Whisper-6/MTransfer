...
# ---------------------- worker_process ----------------------
def worker_process(rank, args, data_batches, config, return_dict, progress):
...
def main():
    ...
    question_field = config.get("question_field")
    for ex in data:
        lang = ex["lang"]
    ...
    for rank in range(args.num_gpus):
        ...
        p = mp.Process(
            ...
            args=(rank, args, rank_data_batches, config, return_dict, progress),
        )
        p.start()
        processes.append(p)
